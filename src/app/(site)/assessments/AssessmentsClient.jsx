...
const assessmentsData = [
  {
    ...
    icon: ClipboardList,
    color: "bg-[#FFB7B2]",
    activeColor: "bg-[#FFB7B2] text-white shadow-[#FFB7B2]/30 shadow-lg",
  ...
  },
  {
    ...
    icon: MessageCircle,
    color: "bg-[#A2D2FF]",
    activeColor: "bg-[#A2D2FF] text-white shadow-[#A2D2FF]/30 shadow-lg",
    inactiveColor: "bg-white text-gray-500 hover:bg-gray-50",
  ...
  },
  {
    ...
    icon: BrainCircuit,
    color: "bg-[#FDFD96]",
    activeColor: "bg-[#FDFD96] text-gray-800 shadow-[#FDFD96]/40 shadow-lg",
    inactiveColor: "bg-white text-gray-500 hover:bg-gray-50",
  ...
  },
  {
    ...
    icon: Activity,
    color: "bg-[#B2FBA5]",
    activeColor: "bg-[#B2FBA5] text-gray-800 shadow-[#B2FBA5]/40 shadow-lg",
    inactiveColor: "bg-white text-gray-500 hover:bg-gray-50",
    ...
    <div className="w-full bg-[#fcfcfc] py-12 md:py-20 px-4 min-h-[85vh]">
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-12 md:mb-20">
          <div className="inline-block px-4 py-1.5 rounded-full bg-dream-blue text-white font-bold text-sm mb-4 tracking-wider shadow-sm">
            ASSESSMENTS
          </div>
        ...
        <div className="flex flex-col lg:flex-row gap-8 lg:gap-12">
              ...
              {assessmentsData.map((tab) => {
                ...
                return (
                  <button
                    ...
                    className={`flex items-center gap-4 px-5 py-4 rounded-2xl text-left transition-all duration-300 whitespace-nowrap lg:whitespace-normal group border ${
                      isActive 
          ...
          <div className="w-full lg:w-2/3">
            <div 
              ...
              className={`bg-white rounded-[2rem] p-6 md:p-10 shadow-xl shadow-gray-100/50 border border-gray-100/80 relative overflow-hidden custom-fade-in duration-500`}
            >
              {/* 백그라운드 그라데이션 장식 */}
              ...
              <div className="flex items-center gap-4 mb-10 relative z-10">
                <div className={`p-4 rounded-2xl ${activeData.color} shadow-sm`}>
                ...
                {activeData.items.map((item, idx) => (
                  <div key={idx} className="group">
                    <h3 className="text-xl font-bold text-gray-900 mb-3 flex items-center gap-3">
                      <span className={`w-2.5 h-2.5 rounded-full ${activeData.dotColor} shadow-sm transition-transform duration-300 group-hover:scale-150`}></span>
                      {item.title}
                    </h3>
                    
                    {item.desc && (
                      <p className="text-gray-600 mb-4 text-[1.05rem] leading-relaxed pl-5 border-l-2 border-gray-100">
                        {item.desc}
                      </p>
                    )}
                      ...
                      <ul className="space-y-3 bg-gray-50/80 hover:bg-gray-50 p-5 rounded-2xl border border-gray-100/80 transition-colors duration-300 ml-5">
                        {item.list.map((listItem, listIdx) => (
                          <li key={listIdx} className="flex items-start gap-3 text-gray-700 text-[0.95rem]">
                            <span className="text-dream-yellow font-extrabold mt-0.5 text-lg leading-none">•</span>
